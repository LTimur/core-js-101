...
function getDigitalRoot(num) {
  const digitsSum = [...String(num)].reduce((sum, digit) => sum + Number(digit), 0);
  if (digitsSum <= 9) {
    return digitsSum;
  }
  return getDigitalRoot(digitsSum);
}
...
function getCommonDirectoryPath(pathes) {
  ...
  if (lastSlashIndex !== -1) {
    commonPath = commonPath.slice(0, lastSlashIndex + 1);
  }
  return commonPath;
}
